    ...
    return data
      .filter(
        (country: any) =>
          ...
          Number.isFinite(Number(country?.lat)) &&
          Number.isFinite(Number(country?.long)),
      )
      .map((country: any) => ({
        name: country.name,
        flag: country.iso2
          ? `https://flagcdn.com/w80/${String(country.iso2).toLowerCase()}.png`
          : "/assets/icons/location-mark.svg",
        coordinates: [Number(country.lat), Number(country.long)],
        value: country.name,
      }));
  } catch (error) {
...
const CreateTrip = ({ loaderData }: Route.ComponentProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let result: (CreateTripResponse & { error?: string }) | null = null;

      try {
        result = raw ? JSON.parse(raw) : null;
      } catch {
        setError(raw || "Failed to create trip.");
        return;
      }
      ...
      else setError("Trip was generated but no trip id was returned.");
    } catch (error) {
      console.error("Error logging trip:", error);
      setError("Unexpected error while creating trip.");
    } finally {
      setLoading(false);
    }
  };
