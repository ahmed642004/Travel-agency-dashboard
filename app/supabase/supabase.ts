...
export const requireAuthenticatedAccountId = async () => {
  const { data: authData, error } = await supabase.auth.getUser();
  if (error) {
    console.error("Supabase: Error reading auth session", error);
...
export const getCurrentUserProfile = async () => {
  try {
    ...
    console.error("Supabase: Error fetching current user profile", error);
    return null;
  }
};
